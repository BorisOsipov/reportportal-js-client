...
class RPClient {
    ...
    constructor(params) {
        ...
        this.isLaunchMergeRequired = params.isLaunchMergeRequired === undefined
            ? false
    ...
    startLaunch(launchDataRQ) {
        ...
        if (launchDataRQ.id) {
            ...
            if (launchDataRQ.tags) {
                launchData.attributes = launchDataRQ.tags.map((value) => (typeof value === 'string' ? { value } : { ...value }));
            }
        ...
        this.map[tempId] = this.getNewItemObj((resolve, reject) => {
            itemObj.promiseStart.then(
                () => {
                    ...
                    requestPromiseFunc(itemObj.realId).then(
                        (response) => {
    ...
    sendLogWithoutFile(itemTempId, saveLogRQ) {
        ...
        const requestPromise = (id) => {
            ...
            return this.restClient.create(url, saveLogRQ, {
                headers: this.headers,
            });
        };
    ...
    finishTestItemPromiseStart(itemObj, itemTempId, finishTestItemData) {
        itemObj.promiseStart.then(
            () => {
                ...
                this.logDebug(`Finish test item ${itemTempId}`);
                this.restClient
                    .update(url, finishTestItemData, { headers: this.headers })
                    .then(
                        (response) => {
